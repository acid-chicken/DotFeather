...
namespace DotFeather.Demo {
  public class LauncherScene : Scene {
    ...
    OnStart(Router router, GameBase game,
      ...
      var titleText =
          new TextDrawable("DotFeather", 56, FontStyle.Normal, Color.White){
              Location = new Vector(24, 24),
          };
      Root.Add(titleText);

      var sampleProgramText = new TextDrawable($"Demo {DemoOS.VERSION}", 24,
      ...
      listView.Location =
          new Vector(16, titleText.Location.Y + titleText.Height + 16);
    ...
    public override void OnUpdate(Router router, GameBase game, DFEventArgs e) {
      Title =
          $"DotFeather Demo - {DemoOS.CurrentDirectory.Name.ToUpperInvariant()}";
    ...
    public void ChangeDirectory(Folder folder) {
      ...
      if (folder.Parent != null) {
        listView.Items.Add(
            new ListViewItem("← ..", folder.Parent.Name.ToUpperInvariant()));
      }

      folder.Files.ForEach(
          el => listView.Items.Add(el switch {
              Folder folder => new ListViewItem(folder.Name.ToUpperInvariant()),
              SceneFile file => new ListViewItem(
                  file.Name.ToUpperInvariant(),
                  file.Description[CultureInfo.CurrentCulture
    ...
    public class ListView : Container {
      ...
      public int Padding {
        ...
        public ListView(IEnumerable<ListViewItem>? items = null)
        {
          ...
          backdrop = new Sprite(t){ZOrder = -4};
          inner = new Container();
          Add(backdrop);
          Add(inner);
        }
        ...
        public override void OnUpdate(GameBase game) {
          ...
          // 範囲外なら無視
          if (!Intersects(DFMouse.Position, Location,
                          Location + new Vector(Width, Height)))
            return;
          ...
          if (innerY <
              -(padding + (ItemHeight + padding + 16) * Items.Count) + Height)
            innerY =
                -(padding + (ItemHeight + padding + 16) * Items.Count) + Height;
        ...
        private bool Intersects(Vector point, Vector topLeft,
                                Vector bottomRight) {
          var (px, py) = point;
          var (tlx, tly) = topLeft;
        ...
        private void UpdateList() {
          ...
          foreach(var item in Items) {
            if (item.Icon is Texture2D ico) {
              var icon =
                  new Sprite(ico){Location = new Vector(padding, ItemHeight),
                                  Width = ItemHeight, Height = ItemHeight};

              inner.Add(icon);
            }
            var text = new TextDrawable(item.Text, ItemHeight, FontStyle.Normal,
                                        Color.White){
            ...
            if (item.Description != null) {
              y += 4;
              var desc = new TextDrawable(item.Description, 12,
                                          FontStyle.Normal, Color.LightGray){
    ...
    public class ListViewItem {
      ...
      public ListViewItem(string text, string? description = null,
                          Texture2D? icon = null) => (Text, Icon, Description) =
