...
public static class CoroutineRunner {
  ...
  internal static void Update() {
    foreach(var (coroutine, obj) in coroutines.Select(c => (c.Key, c.Value))
                .ToArray()) {
  ...
  private static YieldInstruction ToYieldInstruction(object obj) {
    return obj switch {
        YieldInstruction y => y,          IEnumerator ie => Start(ie),
        Task t => t.ToYieldInstruction(), ValueTask t => t.ToYieldInstruction(),
        _ => new WaitUntilNextFrame(),
    };
  }

  private static readonly Dictionary<Coroutine, object?>coroutines =
      new Dictionary<Coroutine, object?>();
}
}
