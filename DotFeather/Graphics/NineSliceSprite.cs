...
public class NineSliceSprite : IContainable, ISizedDrawable {
  ...
  public static NineSliceSprite
  LoadFrom(string path, int left, int top, int right,
           int bottom) => new NineSliceSprite(path, left, top, right, bottom);
  ...
  /// <returns>Generated <see cref="NineSliceSprite"/>。</returns>
  public static NineSliceSprite
  LoadFrom(Stream stream, int left, int top, int right,
           int bottom) => new NineSliceSprite(stream, left, top, right, bottom);
  ...
  public void Destroy() {
    if (internalTexture == null)
      return;
    foreach(var t in internalTexture) { t.Dispose(); }
  }
  ...
  public void Draw(GameBase game, Vector location) {
    ...
    TextureDrawer.Draw(game, Textures[0], location + Location, Scale, Angle,
                       Color);
    TextureDrawer.Draw(game, Textures[1],
                       location + Location +
                           (Vector.Right * WidthOf(0)) * Scale,
                       Scale, Angle, Color, xSpan);
    TextureDrawer.Draw(game, Textures[2],
                       location + Location +
                           (Vector.Right * (WidthOf(0) + xSpan)) * Scale,
                       Scale, Angle, Color);
    TextureDrawer.Draw(game, Textures[3],
                       location + Location +
                           (new Vector(0, HeightOf(0))) * Scale,
                       Scale, Angle, Color, null, ySpan);
    TextureDrawer.Draw(game, Textures[4],
                       location + Location +
                           (new Vector(WidthOf(0), HeightOf(0))) * Scale,
                       Scale, Angle, Color, xSpan, ySpan);
    TextureDrawer.Draw(game, Textures[5],
                       location + Location +
                           (new Vector(WidthOf(0) + xSpan, HeightOf(0))) *
                               Scale,
                       Scale, Angle, Color, null, ySpan);
    TextureDrawer.Draw(game, Textures[6],
                       location + Location +
                           (new Vector(0, HeightOf(0) + ySpan)) * Scale,
                       Scale, Angle, Color, null);
    TextureDrawer.Draw(game, Textures[7],
                       location + Location +
                           (new Vector(WidthOf(0), HeightOf(0) + ySpan)) *
                               Scale,
                       Scale, Angle, Color, xSpan);
    TextureDrawer.Draw(
        game, Textures[8],
        location + Location +
            (new Vector(WidthOf(0) + xSpan, HeightOf(0) + ySpan)) * Scale,
        Scale, Angle, Color, null);
  }
  ...
  private NineSliceSprite(Stream stream, int left, int top, int right,
                          int bottom)
