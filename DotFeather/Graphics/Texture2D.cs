...
public struct Texture2D : IDisposable {
  ...
  /// <summary>
  /// Initialize a new instance of  <see cref="Texture2D"/> with the specified
  /// handle and size.
  /// </summary>
  ...
  public static Texture2D LoadFrom(Image bmp) {
    using(bmp) using(var img = bmp.CloneAs<Rgba32>()) {
  ...
  public static Texture2D Create(byte[] bmp, int width, int height) {
    ...
    GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba, width,
                  height, 0, OpenTK.Graphics.OpenGL.PixelFormat.Rgba,
                  PixelType.UnsignedByte, bmp);
  ...
  /// </summary>
  /// <returns>Generated texture.</returns>
  public static Texture2D CreateSolid(SD.Color color, VectorInt size) =>
      CreateSolid(color, size.X, size.Y);

  /// <summary>
  /// Reads an image file and cuts it out in order from the top left at the
  /// specified size.
  /// </summary>
  /// <returns>All the image data cut out.</returns>
  /// <param name="path">File path</param>
  /// <param name="horizonalCount">The number of images in the horizontal
  /// direction.</param> <param name="verticalCount">The number of images in the
  /// vertical direction.</param> <param name="sizeOfCroppedImage">The size of a
  /// piece of image.</param>
  public static Texture2D[] LoadAndSplitFrom(string path, int horizonalCount,
                                             int verticalCount,
                                             VectorInt sizeOfCroppedImage) {
    return LoadAndSplitFrom(Image.Load(path), horizonalCount, verticalCount,
                            sizeOfCroppedImage);
  }
  ...
  /// vertical direction.</param> <param name="sizeOfCroppedImage">The size of a
  /// piece of image.</param>
  public static Texture2D[] LoadAndSplitFrom(Stream stream, int horizonalCount,
                                             int verticalCount,
                                             VectorInt sizeOfCroppedImage) {
    return LoadAndSplitFrom(Image.Load(stream), horizonalCount, verticalCount,
                            sizeOfCroppedImage);
  }
  ...
  public static Texture2D[] LoadAndSplitFrom(string path, int left, int top,
  ...
  /// <param name="right">Pixel value from right.</param>
  /// <param name="bottom">Pixel value from bottom.</param>
  public static Texture2D[] LoadAndSplitFrom(Stream stream, int left, int top,
                                             int right, int bottom) {
    return LoadAndSplitFrom(Image.Load(stream), left, top, right, bottom);
  }

  private static Texture2D[] LoadAndSplitFrom(Image bmp, int horizonalCount,
                                              int verticalCount,
                                              VectorInt sizeOfCroppedImage) {
    using(bmp) using(var img = bmp.CloneAs<Rgba32>()) {
      ...
      for (int y = 0; y < verticalCount; y++) {
        for (int x = 0; x < horizonalCount; x++) {
          (var px, var py) =
              (x * sizeOfCroppedImage.X, y * sizeOfCroppedImage.Y);
          ...
          using var cropped = img.Clone(
              ctx => ctx.Crop(new Rectangle(px, py, sizeOfCroppedImage.X,
                                            sizeOfCroppedImage.Y)));
  ...
  private static Texture2D[] LoadAndSplitFrom(Image bitmap, int left, int top,
                                              int right, int bottom) {
    using(bitmap) using(var img = bitmap.CloneAs<Rgba32>()) {
      if (left > img.Width)
      ...
      Rectangle[] atlas = {
          ...
          new Rectangle(left, top, img.Width - left - right,
                        img.Height - top - bottom),
          new Rectangle(img.Width - right, top, right,
                        img.Height - top - bottom),
          new Rectangle(0, img.Height - bottom, left, bottom),
          new Rectangle(left, img.Height - bottom, img.Width - left - right,
                        bottom),
