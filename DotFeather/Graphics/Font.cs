...
public class Font {
  ...
  public static Font GetDefault(float size = 16,
                                FontStyle style = FontStyle.Normal) {
    return new Font(defaultFont, "__DOTFEATHER_SYSTEM_EMBEDDED_FONT_MPLUS__",
                    size, style);
  }
  ...
  /// </summary>
  /// <param name="path">relative path to the font, or font-family name of
  /// system fonts.</param> <param name="size">font size by pixel unit.</param>
  /// <param name="style">font style.</param>
  public Font(string path, float size = 16,
              FontStyle style = FontStyle.Normal) {
    Path = path;
    Id = path;
    Size = size;
  ...
  /// <param name="style">font style.</param>
  public Font(Stream stream, string id, float size = 16,
              FontStyle style = FontStyle.Normal) {
    Id = id;
    Stream = stream;
    Size = size;
  ...
  public override bool Equals(object? obj) {
    return obj is Font font && Path == font.Path &&
           EqualityComparer<Stream?>.Default.Equals(Stream, font.Stream) &&
           Size == font.Size && FontStyle == font.FontStyle;
  }
  ...
  public override int GetHashCode() {
    ...
    hashCode = hashCode * -1521134295 +
               EqualityComparer<Stream?>.Default.GetHashCode(Stream);
  ...
  private static readonly Stream defaultFont =
      typeof(Font).Assembly.GetManifestResourceStream(
          typeof(Font).Assembly.GetManifestResourceNames().First(
              n => n.Contains("font.ttf")));
}
}
