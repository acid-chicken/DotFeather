...
public class TextDrawable : TextureDrawableBase {
  ...
  /// Initialize a new instance of <see cref="TextDrawable"/>.
  /// </summary>
  public TextDrawable(string text, float fontSize = 16,
                      FontStyle fontStyle = FontStyle.Normal,
                      SD.Color? color = default) {
    this.text = text;
  ...
  public void UpdateTexture() {
    ...
    using var img =
        new Image<Rgba32>((int) size.Width + 8, (int) size.Height + 8);
    var col = Color ?? SD.Color.Black;
    var isColor =
        SixLabors.ImageSharp.Color.FromRgba(col.R, col.G, col.B, col.A);

    if (BorderColor != null) {
      var bc = BorderColor ?? SD.Color.Black;
      var isBorderColor =
          SixLabors.ImageSharp.Color.FromRgba(bc.R, bc.G, bc.B, bc.A);
      img.Mutate(ctx => ctx.DrawText(Text, f, new SolidBrush(isColor),
                                     new Pen(isBorderColor, BorderThickness),
                                     SixLabors.Primitives.PointF.Empty));
    } else {
      img.Mutate(ctx => ctx.DrawText(Text, f, isColor,
                                     SixLabors.Primitives.PointF.Empty));
    }
  ...
  /// Dispose this object.
  /// </summary>
  public override void Destroy() { Texture.Dispose(); }
  ...
  private SF.Font ResolveFont(Font f) {
    ...
    if (fontCache.ContainsKey(f.Id)) {
      ...
      throw new ArgumentException(
          "Font class must have either a path or a stream.");
    }
    fontCache[f.Id] = family;
    return new SF.Font(family, f.Size, (SF.FontStyle) f.FontStyle);
  }

  private static readonly Dictionary<object, SF.FontFamily>fontCache =
      new Dictionary<object, SF.FontFamily>();
