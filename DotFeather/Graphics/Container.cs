...
/// <summary>
/// 他の <see cref="IDrawable"/>
/// オブジェクトを格納し、相対位置に描画するオブジェクトです。
/// </summary>
public class Container : ISizedDrawable,
                         IContainable,
                         IUpdatable,
                         IList<IDrawable> {
  /// <summary>
  /// この <see cref="T:DotFeather.Drawable.IDrawable"/>
  /// の描画優先順位を取得または設定します。数値が低いほど奥に描画されます。
  /// </summary>
  public int ZOrder {
    get;
    set;
  ...
  /// <summary>
  /// Get or set whether this container is trimmable. If true, this container
  /// draws children with trimming within rectangular range of this container.
  /// </summary>
  ...
  /// コンテナをソートします。
  /// </summary>
  public void Sort() => Children.Sort((d1, d2) => d1.ZOrder < d2.ZOrder ? 1
                                      : d1.ZOrder > d2.ZOrder ? - 1
                                      : countMap[d1] < countMap[d2] ? 1
                                      : -1);
...
#region other inherited members
  ...
  /// </summary>
  public void CopyTo(IDrawable[] array,
                     int arrayIndex) => Children.CopyTo(array, arrayIndex);
  ...
  private List<IDrawable>Children { get; }
  = new List<IDrawable>(10000);
  private readonly Dictionary<IDrawable, int>countMap =
      new Dictionary<IDrawable, int>();
