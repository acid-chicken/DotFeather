...
public class WaveAudioSource : IAudioSource {
  ...
  /// <summary>
  /// Initialize a new instance of <see cref="WaveAudioSource"/> class with
  /// specified file path.
  /// </summary>
  /// <param name="path">ファイルパス。</param>
  public WaveAudioSource(string path) {
    store =
        LoadWave(File.OpenRead(path), out channels, out bits, out sampleRate);
  }
    ...
    private short
    Pull(ref int currentSample) => (short)(store[currentSample++] * 128);
    private short Pull16(ref int currentSample) =>
        (short)(store[currentSample++] | (store[currentSample++] << 8));

    private static byte[] LoadWave(Stream stream, out int channels,
                                   out int bits, out int rate) {
      if (stream == null)
      ...
      if (bitsPerSample != 8 && bitsPerSample != 16)
        throw new NotSupportedException(
            "DotFeather only supports 8bit or 16bit per sample.");

      if (fileChannels < 1 || 2 < fileChannels)
        throw new NotSupportedException(
            "DotFeather only supports 1ch or 2ch audio.");
