...
public abstract class OpenTKManagedHandleBase<T>: IDisposable {
  ...
  /// <summary>
  /// <see cref="OpenTKManagedHandleBase{T}"/>
  /// クラスの新しいインスタンスを初期化します。
  /// </summary>
  protected OpenTKManagedHandleBase() { Handle = GenerateHandle(); }
  ...
  operator T(OpenTKManagedHandleBase<T>handle) => handle.Handle;

  /// <summary>
  /// 整数をハンドルに暗黙変換します。
  /// </summary>
  public static implicit
  operator OpenTKManagedHandleBase<T>(T handle) => handle;
